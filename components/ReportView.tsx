...
import { DIMENSION_META, money, scoreTone, toneText, verdictTone, cn } from "@/lib/ui";
  ...
  return (
    <div className={cn("space-y-6", print && "mx-auto max-w-3xl")}>
      ...
      <div className="card overflow-hidden p-0">
        <div className="bg-grid flex flex-col items-center gap-6 p-6 sm:flex-row sm:p-8">
          <ScoreGauge score={overall.overall_score} size={172} sublabel={overall.rating} />
          <div className="flex-1 text-center sm:text-left">
            <Badge tone={vtone} className="mb-2">{overall.emoji} {overall.recommendation}</Badge>
            <h1 className="text-2xl font-bold leading-snug tracking-tight">{input.business_idea}</h1>
            <p className="mt-1 text-sm text-muted">{input.target_customer} · {input.location}</p>
            ...
            <div key={s} className="rounded-xl border border-border bg-surface-2 p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-lg">{DIMENSION_META[s].icon}</span>
                  <span className="text-sm font-medium">{DIMENSION_META[s].label}</span>
                </div>
                <span className={cn("text-lg font-bold tabular-nums", toneText[scoreTone(cs[s])])}>{cs[s]}</span>
              </div>
              <div className="mt-3">
                <ScoreBar score={cs[s]} />
              </div>
      ...
      <Section title="Executive summary">
        <p className="whitespace-pre-line text-sm leading-relaxed text-muted">{report.executive_summary}</p>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <ListCard title="Key findings" items={report.key_findings} tone="brand" />
          <ListCard title="Critical success factors" items={report.critical_success_factors} tone="go" />
        </div>
      </Section>
      ...
      <Section title="Financial analysis" subtitle="Computed deterministically from your numbers">
        ...
        {result.stages.financial && (
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <ListCard title="Strengths" items={result.stages.financial.strengths} tone="go" />
            <ListCard title="Concerns" items={result.stages.financial.concerns} tone="stop" />
          </div>
        )}
      </Section>
      ...
      <Section
        title="Risk register"
        subtitle={`Overall risk ${riskScoring.overallRiskScore}/100 · ${riskScoring.riskLevel}`}
      >
            ...
            <div key={i} className="rounded-xl border border-border bg-surface-2 p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="text-right">
                  <div className={cn("text-sm font-bold tabular-nums", toneText[scoreTone(100 - (r.priority ?? 0))])}>
                    {r.priority}
                  </div>
                  <div className="text-[0.65rem] text-faint">priority</div>
                </div>
              </div>
              <div className="mt-2 grid gap-2 text-xs text-muted sm:grid-cols-2">
                <div><span className="text-faint">Impact</span> {r.impact}% · <span className="text-faint">Likelihood</span> {r.probability}%</div>
            ...
            <details key={s} className="group rounded-xl border border-border bg-surface-2 p-4" open={print}>
              <summary className="flex cursor-pointer list-none items-center justify-between">
                <span className="flex items-center gap-2 font-medium">
                  <span>{DIMENSION_META[s].icon}</span> {DIMENSION_META[s].label}
                </span>
                <span className={cn("text-sm font-bold", toneText[scoreTone(cs[s])])}>{cs[s]}/100</span>
              </summary>
      ...
      <Section title="Recommendation & next steps">
        <p className="whitespace-pre-line text-sm leading-relaxed text-muted">{report.conclusion}</p>
        {result.conditions.length > 0 && (
          <div className="mt-4">
            <ListCard title="Conditions to address first" items={result.conditions} tone="warn" />
          </div>
        )}
        <div className="mt-4">
          <ListCard title="Next steps" items={result.nextSteps} tone="brand" numbered />
        </div>
      </Section>
...
function Metric({ label, value, tone }: { label: string; value: string; tone?: "go" | "stop" }) {
  return (
    <div className="rounded-xl bg-surface/70 p-3 text-center sm:text-left">
      <div className={cn("text-lg font-bold", tone === "go" ? "text-go" : tone === "stop" ? "text-stop" : "text-ink")}>{value}</div>
...
function Row({ k, v, tone }: { k: string; v: string; tone?: "go" | "stop" | "warn" }) {
  return (
    <tr>
      ...
      <td className={cn("py-2.5 text-right font-semibold tabular-nums", tone === "go" ? "text-go" : tone === "stop" ? "text-stop" : tone === "warn" ? "text-warn" : "")}>{v}</td>
    </tr>
  );
}
...
function ListCard({
  ...
  title: string;
  items: string[];
  tone: "go" | "stop" | "warn" | "brand";
  numbered?: boolean;
}) {
  const dot = tone === "go" ? "text-go" : tone === "stop" ? "text-stop" : tone === "warn" ? "text-warn" : "text-brand";
  return (
    <div className="rounded-xl border border-border bg-surface-2 p-4">
      <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-faint">{title}</div>
      <ul className="space-y-1.5 text-sm text-muted">
        {items.map((it, i) => (
          <li key={i} className="flex gap-2">
            <span className={cn("shrink-0 font-bold", dot)}>{numbered ? `${i + 1}.` : "•"}</span>
            <span>{it}</span>
          </li>
        ))}
      </ul>
